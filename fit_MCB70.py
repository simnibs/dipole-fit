...
coilmesh = trimesh.load_mesh('coildata/MagVenture_MC-B70.stl')
...
data = np.load('coildata/MC-B70_data.npz')

#scale positions to meters and cast to 64 bit float for precision
...
#recommended but will use quite a lot of memory (~30GB), subsampling with a
#factor of 3 will use 32k measurement points
subsample_factor = 3
...
#the upper part (CT scan shows only wires there), this further restiction does not really influence the fit
#but creates a more compact coilmodel and reduces memory requirements. Note that fitting here requires
#a lot of memory (~30 GB), this can be reduced by decreasing the number of measurements or dipoles positions
# if desired. Increasing the number of splits will also reduce memory footprint.

#run coilfitting with two splits with free dipole directions, note that B field is a list of one element
#expected runtime on modern computer (year 2021) ~10 min
...
ccdfile='coildata/MCB70_full.ccd'
